...
tribe_api = f"{tribe_end_point}/tribe/f8ce5080-2523-42f0-ab0b-85f90c57ee7e/join"
    ...
    async def refresh_token_func(self, http_client: aiohttp.ClientSession, proxy):
        ...
        try:
            ...
            if refresh.status == 200:
                ...
                logger.warning(f"{self.session_name} | <yellow>Failed to refresh token: <red>{refresh.status}</red></yellow>")
                return False
                ...
                if can_run:
                    ...
                    user_info = f"""
                      ====<cyan>{self.session_name}</cyan>====
                            USER INFO: BP Balance: <cyan>{self.user_balance}</cyan> BP | Total play passes: <cyan>{self.play_passes}</cyan> | Farming: <red>{is_farming}</red> | Dogs drop: <red>{self.dogs_eligible}</red>
                            FRENS INFO: Total invited: <cyan>{friend_info['usedInvitation']}</cyan> | Amount for claim: <cyan>{friend_info['amountForClaim']}</cyan> | Can claim: <red>{friend_info['canClaim']}</red>
                            """
